function result = SkyPlot(el_matrix,az_matrix)
...
for index2=1:length(diff_sign_vec_ind)-1
    ...
    set(h_line,'Color',cmap(index1,:));
    if index2==1
        h_init=plot(x_values(1),y_values(1),'o','Color',cmap(index1,:),'MarkerSize',8);
    ...
    if index2==(length(diff_sign_vec_ind)-1)
        h_end=plot(x_values(end),y_values(end),'x','Color',cmap(index1,:),'MarkerSize',8);
        if sign_type==1
